...
export default class RealTimeEngine {
    ...
    simulate(untilSeconds: number = this.matchLengthSeconds + 15 * 60): MatchSnapshot[] {
        if (!this.gameStarted) {
            this.start();
        }

        // 允许补时：实际终场由 handleTimeBoundaries 的 fullTimeWithAdded 判定，
        // untilSeconds 只作为防死循环的安全上限（默认 90 分钟 + 15 分钟补时缓冲）。
        const endTime = untilSeconds;
