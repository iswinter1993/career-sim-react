...
check(approx(effects.actionModifiers.pass, 0.2), `one-twos + killerBalls pass = 0.3 + -0.1 ≈ 0.2, got ${effects.actionModifiers.pass}`);
...
check(approx(wideEffects.movementModifiers.stayWide, 0.1), `cuts_inside + hugs_line stayWide ≈ 0.1, got ${wideEffects.movementModifiers.stayWide}`);
check(approx(wideEffects.movementModifiers.cutInside, 0.2), `cuts_inside + hugs_line cutInside ≈ 0.2, got ${wideEffects.movementModifiers.cutInside}`);
...
check(fallback === 1.0, 'no role/strategy fallback = 1.0');
...
check(b2bMove.staminaDrain > anchorMove.staminaDrain, `B2B staminaDrain > Anchor`);
