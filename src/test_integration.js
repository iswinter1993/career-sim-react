...
const PITCH_DEFAULT = PITCH_DEFAULT || { pitchWidth: 680, pitchHeight: 1050, goalWidth: 50 };
...
for (const tc of compatChecks) {
  const result = validateTacticsCompatibility(tc);
...
check(ap1Derived.saves === 6, `AP1 6 saves: ${ap1Derived.saves}`);
...
// 9E: Unknown role in getRoleModifier
const unknownRole = getRoleModifier('NONEXISTENT_ROLE');
check(unknownRole === null || unknownRole === undefined, `unknown role returns null/undefined: ${unknownRole}`);
...
check(bareGK >= 5.5 && bareGK <= 6.8, `bare GK rating in [5.5, 6.8]: ${bareGK}`);
...
// 10A: buildMatchTactics → injectTacticsIntoTeam → createMatch pipeline
const testSquad = {
  players: [
...
// ST with goal + shots should lead
check(e9Rate > e4Rate, `ST (${e9Rate}) > CB (${e4Rate}) — attackers rate higher`);
...
const contradictResult = validateTacticsCompatibility({
  formation: '4-4-2',
  strategy: {
    ...STYLE_PRESETS.gegenpress,
    defensiveLine: 'deep',  // override — contradicts gegenpress
  },
  mentality: 'balanced',
});
...
const tempoDirectResult = validateTacticsCompatibility({
  formation: '4-4-2',
  strategy: {
    ...STYLE_PRESETS.tiki_taka,
    passingDirectness: 'very_direct',  // contradicts tiki_taka's short passing
  },
  mentality: 'balanced',
});
